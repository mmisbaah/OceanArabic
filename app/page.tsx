...
import { learningLevels, selectedLearningLevel } from "./learning-levels";
import { ks2LessonsFor, ks2PracticeFor, ks2GameFor, ks2UnitsFor } from "./ks2-content";
import { Ks2Workshop } from "./ks2-workshop";
...
export default function Home() {
  ...
  const lessonBank=grade===1?grade1LessonsFor(difficulty):grade===2?grade2LessonsFor(difficulty):grade===3?grade3LessonsFor(difficulty):ks2LessonsFor(grade,difficulty);
  ...
  const practiceAvailable=grade<=5;
  const quiz=grade===1?grade1PracticeFor(difficulty,quizLevel):grade===2?grade2PracticeFor(difficulty,quizLevel):grade===3?grade3PracticeFor(difficulty,quizLevel):ks2PracticeFor(grade,difficulty,quizLevel), question=quiz[quizIndex];
  const practicePhase=grade===1?grade1PracticePhase(difficulty,quizLevel):grade===2?grade2PracticePhase(difficulty,quizLevel):grade===3?grade3PracticePhase(difficulty,quizLevel):ks2UnitsFor(grade)[quizLevel-1]?.title;
  const practiceLevelLabel=(level:number)=>grade>=4?ks2UnitsFor(grade)[level-1]?.arabic:grade===1?(level<=7?"حُرُوف":level<=14?"كَلِمَات":"كُلُّ المَهَارَات"):grade===2?(level<=7?"جُمَل":level<=14?"قَوَاعِد":"إِتْقَان"):(level<=7?"تَرَاكِيب":level<=14?"قِرَاءَة":"إِتْقَان");
  ...
  const gameAvailable=grade<=5,rounds=grade===1?grade1GameFor(difficulty,gameLevel):grade===2?grade2GameFor(difficulty,gameLevel):grade===3?grade3GameFor(difficulty,gameLevel):ks2GameFor(grade,difficulty,gameLevel),round=rounds[gameIndex];
  const currentGamePhase=grade===1?grade1GamePhase(difficulty,gameLevel):grade===2?grade2GamePhase(difficulty,gameLevel):grade===3?grade3GamePhase(difficulty,gameLevel):ks2UnitsFor(grade)[gameLevel-1]?.title;
  const gameLevelLabel=(level:number)=>grade>=4?ks2UnitsFor(grade)[level-1]?.arabic:grade===1?(level<=7?"حُرُوف":level<=14?"كَلِمَات":"مُحَادَثَة"):grade===2?(level<=7?"جُمَل":level<=14?"قَوَاعِد":"إِتْقَان"):(level<=7?"تَرَاكِيب":level<=14?"تَحَدِّي":"إِتْقَان");
  ...
  const chooseLearningLevel=(id:number)=>{const selected=learningLevels.find(level=>level.id===id);if(!selected||actionLock.current)return;resetActivityViews();audioRef.current?.pause();if("speechSynthesis" in window)window.speechSynthesis.cancel();setAudioNotice("");setGrade(selected.grade);setDifficulty(selected.difficulty);setActive("Home");history.replaceState({},"","/");if(learner){const updated={...learner,grade:selected.grade,difficulty:selected.difficulty};setLearner(updated);safeWrite("oceanarabic-learner",updated)}};
  const levelPicker=<label className="learning-level-picker">Learning level<select aria-label="Learning level" value={currentLevel?.id||0} onChange={e=>chooseLearningLevel(Number(e.target.value))}>{!currentLevel&&<option value={0}>Saved route · Grade {grade} {difficulty}</option>}{learningLevels.map(level=><option value={level.id} key={level.id}>Level {level.id} · Grade {level.grade} · {level.title}</option>)}</select></label>;
  ...
  if(!learner)return <main className="entry-page"><section className="entry-card"><img src="/assets/nooru.png" onError={imageFallback("/assets/avatars/avatar-01.png")} alt="Nooru the OceanArabic dolphin"/><p className="arabic entry-salam" dir="rtl">السَّلَامُ عَلَيْكُمْ</p><p className="eyebrow">WELCOME TO OCEANARABIC</p><h1>Ready for an Arabic adventure?</h1><p>Choose your learner and Nooru will guide the way.</p><label>Your name<input value={entryName} onChange={e=>setEntryName(e.target.value)} placeholder="Type your name" maxLength={20}/></label><div className="login-choice">{levelPicker}<p>{currentLevel?.description}</p></div><fieldset><legend>Choose your avatar</legend><div className="avatar-picker">{avatars.map((a,i)=><button key={a} aria-label={`Avatar ${i+1}`} onClick={()=>setEntryAvatar(a)} className={entryAvatar===a?"on":""}><img src={a} onError={imageFallback("/assets/avatars/avatar-01.png")} alt=""/></button>)}</div></fieldset><button className="enter-button" disabled={!entryName.trim()} onClick={enterApp}>Start with Nooru →</button></section></main>;
  return <main className={`app-shell page-${active.toLowerCase()}`}>
    <header className="topbar">
      <div className="header-left"><img className="deck-mascot" src="/assets/nooru.png" onError={imageFallback("/assets/avatars/avatar-01.png")} alt="Nooru mascot"/><div className="avatar"><img src={learner.avatar} onError={imageFallback("/assets/avatars/avatar-01.png")} alt={`${learner.name} avatar`}/></div><div className="learner-label"><b>Hello, {learner.name}</b><small>{currentLevel?`Level ${currentLevel.id}`:`Grade ${grade}`} · {currentLevel?.title||difficulty}</small></div></div>
      <div className="status-stack" aria-label="Learning rewards"><span title="Stars">⭐ {totalStars}</span><span title="Rewards">🏅 {unlocked.length}</span><span title="Learning streak">🔥 {completedActivities?1:0}</span></div>
      <div className="header-actions"><button aria-label="Reset progress" onClick={()=>setResetOpen(true)}>🫧<small>Reset</small></button><button aria-label="Log out" onClick={logout}>🚪<small>Logout</small></button></div>
    </header>
    <div className="level-switcher">{levelPicker}<span>Seven levels · Grades 1–5</span></div>
    ...
    <section className="hero">
      <div className="copy">
        ...
        <p>From first letters to confident reading and writing: explore seven Arabic learning levels with Nooru.</p>
    ...
    <section className="preview">
      ...
      <div className="seven-level-map">{learningLevels.map(level=><button key={level.id} aria-pressed={currentLevel?.id===level.id} className={currentLevel?.id===level.id?"on":""} onClick={()=>chooseLearningLevel(level.id)}><span>{level.id}</span><div><small>GRADE {level.grade}</small><b>{level.title}</b><p>{level.description}</p></div></button>)}</div><div className="coming"><span>🐚</span><div><b>Choose your next Arabic adventure</b><p>Learn letters, practise sounds, play games and collect rewards with Nooru.</p></div></div>
    </section>
    ...
    <section className="game-engine">
      <div className="game-head"><div><p className="eyebrow">STAGE 6 · GAME ENGINE</p><h2>Nooru’s game islands</h2><p>{grade>=4?"Nine reading quests. Explore each passage and collect stars for comprehension.":"Twenty game sets. Tap, listen, build and race through Arabic."}</p></div><div className="game-bubble">🎮<b>5 rounds</b><small>per level</small></div></div>
      {gameAvailable?<>
      {!gameStarted&&<div className="game-levels">{Array.from({length:activityCount},(_,i)=>i+1).map(level=><button key={level} onClick={()=>openGameLevel(level)} className={`${level<=7?"p1":level<=14?"p2":"p3"}`}><b>{level}</b><small className="arabic">{gameLevelLabel(level)}</small></button>)}</div>}
      {gameStarted&&<div className="activity-scene"><button className="back-levels" onClick={()=>setGameStarted(false)}>← <span className="arabic">المُسْتَوَيَات</span></button><article className="game-player">{!gameDone?<><header><div><b>Set {gameLevel}</b><span>Round {gameIndex+1}/5</span><strong>⭐ {gameScore}</strong></div><p>{currentGamePhase}</p></header><div className={`game-board ${round.kind.toLowerCase().replace(" ","-")}`}><div className="game-kind">{round.kind==="Picture Match"?"🖼️":round.kind==="Sound Hunt"?"🎧":round.kind==="Letter Builder"?"🧩":"⏱️"} {round.kind}</div>{round.seconds&&<div className="timer" aria-live="polite">{timeLeft}s reef challenge</div>}<div className="game-visual">{round.visual}</div>{round.passage&&<div className="quiz-passage arabic" lang="ar" dir="rtl">{round.passage}</div>}<h3>{round.prompt}</h3>{round.spoken&&<div className="game-audio"><button onClick={()=>speak(round.spoken!)}>🔊 Hear</button><button onClick={()=>speak(round.spoken!,true)}>🐢 Slower</button></div>}{round.pieces&&<div className="game-pieces">{round.pieces.map((piece,i)=><span key={`${piece}-${i}`} className="arabic">{piece}</span>)}</div>}<div className="game-options">{round.options.map(option=><button key={option} onClick={()=>playChoice(option)} className={`${gameChoice===option?gameFeedback:""}`}><b className="arabic" dir="rtl">{option}</b></button>)}</div><button className="game-hint" onClick={()=>setGameHint(!gameHint)}>💡 {gameHint?round.hint:"Show hint"}</button>{gameFeedback&&<div className={`game-feedback ${gameFeedback}`}>{gameFeedback==="right"?"أَحْسَنْتَ! Splash-tastic!":gameChoice==="__timeout__"?"Time’s up! Nooru will show the next round.":"Nice try! Nooru will show the next round."}</div>}</div></>:<div className="game-result"><span>{"⭐".repeat(gameStars)}</span><h3>Island level complete!</h3><p>You won <b>{gameScore} of 5</b> rounds.</p><button onClick={()=>openGameLevel(gameLevel)}>Play again</button>{gameLevel<activityCount&&gameScore>=3?<button className="next-game" onClick={()=>openGameLevel(gameLevel+1)}>Next island →</button>:gameScore<3?<p>Win 3 rounds to sail onward.</p>:null}</div>}</article></div>}
    ...
    <section className="quiz-engine">
      <div className="quiz-head"><div><p className="eyebrow">PRACTICE SETS</p><h2>{grade>=4?"Nine passage practice sets":"Twenty guided practice sets"}</h2><p>{grade>=4?"Read for meaning, find evidence and check language in context. Each set has five questions.":"Each five-question set follows your learning route, with pictures, listening and hidden hints."}</p></div><div className="quiz-medal">🏅<b>5 questions</b><small>each level</small></div></div>
      {practiceAvailable?<>
      {!quizStarted&&<div className="quiz-levels">{Array.from({length:activityCount},(_,i)=>i+1).map(level=><button key={level} onClick={()=>openQuizLevel(level)} className={`${level<=7?"p1":level<=14?"p2":"p3"}`}><span>{level}</span><small className="arabic">{practiceLevelLabel(level)}</small></button>)}</div>}
    ...
    <section className="progress-dashboard">
      ...
      <div className="progress-summary"><article><span>📖</span><b>{selectedLessons.length}/9</b><small>Lessons</small><i style={{width:`${Math.min(100,selectedLessons.length/9*100)}%`}}/></article><article><span>❓</span><b>{selectedQuizKeys.length}/{activityCount}</b><small>Practice sets</small><i style={{width:`${Math.min(100,selectedQuizKeys.length/activityCount*100)}%`}}/></article><article><span>🎮</span><b>{selectedGameKeys.length}/{activityCount}</b><small>Game sets</small><i style={{width:`${Math.min(100,selectedGameKeys.length/activityCount*100)}%`}}/></article><article><span>🏅</span><b>{unlocked.length}/6</b><small>Badges</small><i style={{width:`${unlocked.length/6*100}%`}}/></article></div>
      <div className="voyage-card"><div><b>Grade {grade} · {difficulty}</b><span>{completedActivities} of {9+activityCount*2} activities complete</span></div><div className="voyage-track"><i style={{width:`${Math.min(100,completedActivities/(9+activityCount*2)*100)}%`}}/><span>🐬</span></div><p>{completedActivities===0?`Begin the Grade ${grade} ${difficulty} route—Nooru is waiting!`:completedActivities<25?"Great swimming! Keep following the learning bubbles.":"You are crossing the final reef!"}</p></div>
    </section>

    <section className={`lesson-prototype ${showLesson?"lesson-detail-page":"lesson-catalogue-page"}`}>
      {!showLesson?<>
      <div className="lesson-head"><div><p className="eyebrow">GRADE {grade} · {difficulty.toUpperCase()}</p><h2>{grade>=4?currentLevel?.title||`Grade ${grade} Arabic`:grade===1?difficulty==="Easy"?"Alphabet Island":difficulty==="Medium"?"Word Bridge":"Grade 1 Voyage":grade===2?difficulty==="Easy"?"Sentence Lagoon":difficulty==="Medium"?"Grammar Reef":"Heritage Harbour":difficulty==="Easy"?"Noun Atoll":difficulty==="Medium"?"Sentence Sea":"Time and Compass Bay"}</h2><p>Choose a lesson to open its learning adventure.</p></div><img src="/assets/nooru.png" alt="Nooru, the Arabic guide"/></div>
      ...
      <article className="lesson-player"><header><div><p>LESSON {lesson.id} · STEP {subIndex+1} OF 5</p><h3>{lesson.title}</h3><div className="arabic" dir="rtl">{lesson.arabicTitle}</div></div><div className="step-tabs">{lesson.subLessons.map((s,i)=><button onClick={()=>openLesson(lessonId,i)} className={i===subIndex?"on":""} key={s.step}><span>{i+1}</span>{s.step}</button>)}</div></header>
        ...
        {grade>=4?<div className="ks2-reading">{subIndex===1?<details key={`${grade}-${difficulty}-${lessonId}`}><summary>Show the transcript after listening</summary><p className="arabic" lang="ar" dir="rtl">{sub.examples[0].word}</p></details>:<p className="arabic" lang="ar" dir="rtl">{sub.examples[0].word}</p>}<button onClick={()=>speak(sub.examples[0].word)}>🔊 Read aloud</button><button onClick={()=>speak(sub.examples[0].word,true)}>🐢 Slower</button></div>:<div className="visual-cards">{sub.examples.map(ex=><button key={ex.word} onClick={()=>speak(ex.word)}><span>{ex.visual}</span><b className="arabic" dir="rtl">{ex.letter}</b><small className="arabic" dir="rtl">{ex.word}</small><i>🔊 Tap</i></button>)}</div>}
        ...
        {grade>=4&&subIndex===4&&<Ks2Workshop key={`${grade}-${difficulty}-${lessonId}`} unit={ks2UnitsFor(grade)[lessonId-1]} grade={grade} difficulty={difficulty} lessonId={lessonId} learnerKey={learner.name}/>}

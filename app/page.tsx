...
import { learningLevels, selectedLearningLevel, levelForProfile } from "./learning-levels";
...
export default function Home() {
  ...
  const choosePlacement=(chosenGrade:number,challenge:string)=>{if(actionLock.current)return;const selected=levelForProfile(chosenGrade,challenge);resetActivityViews();audioRef.current?.pause();if("speechSynthesis" in window)window.speechSynthesis.cancel();setAudioNotice("");setProfileGrade(chosenGrade);setProfileChallenge(challenge);setGrade(selected.grade);setDifficulty(selected.difficulty);setActive("Home");history.replaceState({},"","/");if(learner){const updated={...learner,grade:chosenGrade,difficulty:challenge};setLearner(updated);safeWrite("oceanarabic-learner",updated)}};
  ...
  return <main className={`app-shell page-${active.toLowerCase()}`}>
    <header className="topbar">
      <div className="header-left"><img className="deck-mascot" src="/assets/nooru.png" onError={imageFallback("/assets/avatars/avatar-01.png")} alt="Nooru mascot"/><div className="avatar"><img src={learner.avatar} onError={imageFallback("/assets/avatars/avatar-01.png")} alt={`${learner.name} avatar`}/></div><div className="learner-label"><b>Hello, {learner.name}</b><small>G{learner.grade} · {learner.difficulty} · Level {currentLevel?.id}</small></div></div>
